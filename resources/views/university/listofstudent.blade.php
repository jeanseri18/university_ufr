...
<div class="container">
    ...
    <div class="row">
    @foreach ($docs as $doc)
    <div class="col-md-4 content-item docs">
        <div class="card h-100 shadow-sm border-0 card-hover">
            <iframe src="{{ Storage::url($doc->pdf) }}" class="card-img-top" style="height: 300px; width: 100%;" frameborder="0"></iframe>
            <div class="card-body">
                <h5 class="card-title fw-bold">{{ $doc->titre }}</h5>
                <p class="card-text text-muted">{{ Str::limit($doc->details, 100) }}</p>
                <a href="{{ Storage::url($doc->pdf) }}" class="btn btn-primary stretched-link" target="_blank">Voir en plein écran</a>
            </div>
        </div>
    </div>
@endforeach
